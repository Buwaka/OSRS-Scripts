...
public class OSRSUtilities
{
    ...
    public static boolean BankDepositAll(int... Except)
    {
        ...
        if (Except.length > 0)
        {
            ...
            var items = Inventory.all(t -> Arrays.stream(Except).anyMatch(x -> { Logger.log(x + " != " + t.getID()); return x != t.getID();} ));
            for (var item : items)
            {
    ...
    public static boolean Fish(String Action, int ID)
    {
        ...
        while (!Inventory.isFull())
        {
            ...
            Sleep.sleepUntil(() -> (Players.getLocal().getAnimation() == -1 && !Players.getLocal().isMoving()) || Dialogues.inDialogue(),
                             120000);
    ...
    {
        Logger.log("PickupOnAreaExcepts: ");
        var Items = GroundItems.all(t ->
                                            area.contains(t.getTile())
        ...
        {
            Logger.log(i + ": " + Items.get(i).walkingDistance(Players.getLocal().getTile()));
        }
...
//    }
    ...
    public static NPC GetClosestUnoccupiedEnemy(int... IDs)
    {
        final String AttackAction = "Attack";
        return NPCs.closest(t -> Arrays.stream(IDs).anyMatch(x -> x == t.getID())
                && t.hasAction(AttackAction)
                && !t.isInteractedWith()
                && t.canAttack()
                && t.canReach());
    }

    public static NPC GetClosestUnoccupiedEnemy(String... Names)
    {
    ...
    public static Character Fight(Character Foe)
    {
        if(Foe == null)
        {
    ...
    public static boolean FightInteractingCharacter(int Timeout)
    {
    ...
    public static java.awt.geom.Area LootArea = null;
    ...
    {
        Character Foe = GetClosestUnoccupiedEnemy(IDs);
        if(Foe == null)
        {
    ...
    {
        Character Foe = Players.getLocal().getCharacterInteractingWithMe();
        if (!FightInteractingCharacter(Timeout))
        {
        ...
        if (Sleep.sleepUntil(() -> finalFoe.getHealthPercent() <= 0, Timeout))
        {
            return Foe;
        }
        return null;
    ...
    public static boolean IsLootLeft(Area LootArea, int... LootExcepts)
    {
        ...
        for (Tile tile : Tiles)
        {
            if (GroundItems.getForTile(tile).stream().anyMatch(t -> Arrays.stream(LootExcepts).anyMatch(x -> t.getID() == x)))
            {
                return true;
            }
        }
    ...
    public static Boolean Pray(int Timeout, int... IDs)
    {
    ...
    {
        List<World> Wlords = Worlds.all(t -> t.isF2P() && !t.isHighRisk() && !t.isPVP() && t.isNormal() && !t.getDescription().contains("skill total"));
    ...
    public static boolean IsAreaBusy(int max)
    {
        var players = Players.all();
        Logger.log("Playercount in area " + players.size());
    ...
    public static boolean BankingDropPickupMany(Optional<Integer> KeepID,
                                                AbstractMap.SimpleEntry<Integer, Integer>... WithdrawIDs)
    {
        boolean result = true;
    ...
    public static Boolean IsTimeElapsed(long UID, int FunctionID, int ms)
    {
        long ID = UID ^ Byte.toUnsignedLong((byte) FunctionID);
        ...
        TimeStamps.put(ID, System.nanoTime());
        return false;
    }
