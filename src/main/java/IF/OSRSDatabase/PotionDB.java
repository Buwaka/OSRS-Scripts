...
public class PotionDB extends OSRSDataBase
{
    ...
    public static PotionStep GetProfitablePotionSteps(PotionData Potion)
    {
        ...
        while(!ToDo.isEmpty())
        {
            ...
            {
                if(HerbDB.getInstance().isHerb(step.result.base) && HerbDB.getInstance().isDegrimeProfitable(step.result.base))
                {
                    step.result.base = HerbDB.getInstance().GetGrimyHerb(step.result.base).grimy_id;
                }
                int leftPrice = step.base == null
                        ? GetPrice.apply(step.result.base, step.result.base_count)
                        : step.base.GetPrice();

                if(HerbDB.getInstance().isHerb(step.result.ingredient) &&
                   HerbDB.getInstance().isDegrimeProfitable(step.result.ingredient))
                {
                    step.result.ingredient = HerbDB.getInstance().GetGrimyHerb(step.result.ingredient).grimy_id;
                }
