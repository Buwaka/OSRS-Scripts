...
public class FreakyForesterSolver extends RandomSolver implements ChatListener
{
    ...
    public boolean shouldExecute()
    {
        ...
        return (forester != null && forester.getInteractingCharacter() != null &&
                forester.getInteractingCharacter().equals(Players.getLocal())) ||
    ...
    public int onLoop()
    {
        ...
        if(drop)
        {
            ...
            if(Tabs.isOpen(Tab.INVENTORY))
            {
                ...
                if(items.get(0) != null)
                {
                    if(Inventory.interact(items.get(0), "Drop"))
                    {Sleep.sleep(350, 2500);}
                }
            }
        }
        ...
        if(!areaFreak.contains(Players.getLocal().getTile()))
        {
            ...
            if(forester.interact())
            {
                ...
                Sleep.sleepWhile(() -> Client.getGameState().equals(GameState.LOADING),
                                 Calculations.random(8500, 11000));
            }
        }
