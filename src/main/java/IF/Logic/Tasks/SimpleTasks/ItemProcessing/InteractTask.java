...
public class InteractTask extends SimpleTask
{
    ...
    {
        return GetTargetStatic(EnumSet.allOf(InteractableFilter.class), Action, (Integer[])null);
    }
    ...
    public static Entity GetTargetStatic(EnumSet<InteractableFilter> TargetFilter, String Action, Integer... IDs)
    {
        ...
        var first = toFilter.stream()
                            .filter(t -> (IDs == null ||
                                         ...
                                         t.distance() < 10)
                            .min((x, y) -> (int) (x.walkingDistance(Players.getLocal().getTile()) -
    ...
    private Boolean onItem(IScript.ItemAction action, Item item, Item item1)
    {
    ...
    protected int Loop()
    {
        ...
        {
            OSRSUtilities.JumpToOtherWorld(GetScript().onGameTicked());
        }
