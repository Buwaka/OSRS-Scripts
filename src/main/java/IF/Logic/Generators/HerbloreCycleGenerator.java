...
import IF.Logic.Cycles.General.SimpleInventoryProcessCycle;
...
public class HerbloreCycleGenerator
{
    ...
    private static Tuple2<Item, HerbDB.HerbData>[] GetHerbsFromBank(boolean Grimy)
    {
        ...
        {
            if(item != null && HerbDB.getInstance().isHerb(item.getID()))
            {
                Logger.log("Herb found: " + item.getName());
                if(HerbDB.getInstance().isGrimyHerb(item.getID()))
                {
                    GrimyHerbs.add(new Tuple2<>(item, HerbDB.getInstance().GetHerbData(item.getID())));
                }
                else
                {
                    CleanHerbs.add(new Tuple2<>(item, HerbDB.getInstance().GetHerbData(item.getID())));
                }
            }
        }
